...
local type_qualifier = word_match{
  -- C11 6.7.3
  'const', 'restrict', 'volatile', '_Atomic', 'atomic',
}
...
local true_false = word_match{
   'true', 'false',
   'TRUE', 'FALSE',
}

local stdio_files = word_match{
   'stdin', 'stdout', 'stderr',
}

local constant = token(l.CONSTANT, word_match{ 'NULL' } + errno
                 + preproc_macros + true_false + stdio_files)
